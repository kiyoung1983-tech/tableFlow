...
### 2026-08-17 · 테스트 명세와 경계 회귀 보완

- 구현 범위, 핵심 정책, 동시성·보안 전략과 남은 운영 결정을 `TableFlow_프로젝트_현황_요약.md`에 정리했다.
- 요구사항을 단위·통합·프런트·운영 자동화와 스테이징 수동 UAT에 연결한 `TableFlow_MVP_테스트_케이스.md`를 추가했다.
- 같은 고객의 다음 예약이 이용 종료 경계에는 허용되지만 기존 테이블 정리 시간은 계속 점유하는지를 통합 테스트로 검증했다.
- 취소와 같은 고객·같은 시간 재예약이 경합해도 최종 활성 중복이 남지 않는지를 통합 테스트로 검증했다.
- 실패 요청의 본문이 바뀌면 프런트가 새 멱등 키를 발급하는지를 화면 테스트로 검증했다.
- 전체 게이트에서 백엔드 54개, 프런트 30개, 운영 스크립트 15개 테스트와 OpenAPI·lint·TypeScript·Vite 빌드가 통과했다.
